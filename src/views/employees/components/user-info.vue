<template>
  <div class="user-info">
    <el-button class="el-icon-printer" style="cursor: pointer;" @click="$router.push('/employees/print/' + userId + '?type=personal')"></el-button>
      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <!-- 放置上传图片 -->
            <UploadImg ref="uploadAvator" :employee-avator="employeeAvator" @onSuccess="uploadAvator" />
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <UploadImg :employee-avator="employeePic" @onSuccess="uploadPicSuccess"  />
        </el-form-item>
...
import { getUserDetailById, getPersonalDetail, updatePersonal, saveUserDetailById } from '@/api'
...
export default {
  ...
  methods: {
    ...
    async saveEmploayeesInfo() {
      try {
        if (this.$refs.uploadAvator.loading) return this.$message.error('头像正在上传中')
    ...
    async getUserBasicInfo() {
      ...
      this.userInfo = res
      if (res.staffPhoto) { this.employeeAvator = res.staffPhoto }
      console.log(res)
    },
    async getPersonalDetail() {
      const res = await getPersonalDetail(this.userId)
      // console.log(11, res)
      if (res.staffPhoto) { this.employeePic = res.staffPhoto }
    },
    uploadAvator(data) {
