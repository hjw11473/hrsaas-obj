<template>
  <div class="job-info">
    ...
    <el-form label-width="220px">
      <div class="block">
    <el-button class="el-icon-printer" style="cursor: pointer;" @click="$router.push('/employees/print/' + userId +'?type=jobinfo')"></el-button>
      ...
      <!-- 从业信息 -->
      <el-form-item>
        <el-button type="primary" @click="savejobsAPI">保存更新</el-button>
...
export default {
  data() {
    return {
      ...
      formData: {
        ...
        closingTimeOfCurrentContract: '', //SS 现合同结束时间
        companyId: '', // 公司ID
  ...
  methods: {
    async getjobsAPI() {
      const data = await getjobs(this.userId)
      this.formData = data
    },
    async savejobsAPI() {
      const data = await savejobs(this.formData)
